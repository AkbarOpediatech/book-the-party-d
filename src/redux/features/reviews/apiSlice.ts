...
export const reviewsApi = createApi({
  ...
  tagTypes: ['Reviews'],
  endpoints: builder => ({
    fetchReviews: builder.query<ServiceResponse, { service?: string; limit?: number; page?: number }>({
      query: ({ service, limit, page } = {}) => {
        const params = service ? { service } : {} // Conditionally include `role` if it exists
        return {
    ...
    // }),
    fetchReviewById: builder.query<SingleServiceResponse, string>({
      query: slug => `/reviews/${slug}`,
      providesTags: (result, error, slug) => [{ type: 'Reviews', slug }]
    }),
    addReview: builder.mutation<ServiceItemPost, Omit<ServiceItemPost, 'id'>>({
      query: newReview => ({
        url: '/reviews',
        method: 'POST',
        body: newReview
    ...
    updateReview: builder.mutation<ServiceItem, Partial<ServiceItem> & Pick<ServiceItem, 'slug'>>({
      query: ({ slug, ...rest }) => ({
        url: `/reviews/${slug}`,
        method: 'PUT',
        body: rest
